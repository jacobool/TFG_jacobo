...
SAVE_FREQ = 10000  # Guardar automáticamente cada 50,000 pasos
...
class GDEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.now = time.time()
        self.player_last_seen = self.now
        self.alive = False
        
        while True:
            frame = self._capture_frame()
            player_y, green_area = detect_player_and_band(frame)
            if green_area > 100:
                self.alive = True
                self.attempt += 1
                # Se comenta el print del intento para mantener limpia la consola
                # print(f"▶ NUEVO EPISODIO → Attempt {self.attempt}")
                break 
            time.sleep(0.05) 
    ...
    def step(self, action):
        self.now = time.time()
        reward = 0.2  # Premio por vivir un frame más
        done = False

        if action == 1:
            # Ya NO hay castigo por saltar. Fomentamos la exploración.
            if (self.now - self.last_action_time > 0.05):  
                pydirectinput.keyDown("space")
                pydirectinput.keyUp("space")
                self.last_action_time = self.now
        ...
        if player_visible:
            ...
            if self.alive and (self.now - self.player_last_seen) >= DEATH_TIMEOUT:
                # print(f"💀 MUERTE → Attempt {self.attempt} terminado")
                self.alive = False
                reward = -10.0  # Castigo por morir
                done = True
            else:
                done = False
                reward = 0.0
        ...
        obs = self._get_obs(frame)
        truncated = False
        info = {"attempt": self.attempt, "green": green_area, "y": player_y}
    ...
    def _get_obs(self, frame=None):
        ...
        # Expandimos dimensión para que cumpla con shape=(84, 84, 1)
...
if __name__ == "__main__":
    ...
    model.learn(total_timesteps=150000, callback=save_callback) 
    
    # 5. AL TERMINAR
    print("\n📊 Entrenamiento finalizado por completo. Generando la gráfica final...")
    save_plots(save_callback.history, show=True) # Aquí SÍ abrimos la ventana de la gráfica
    
    model.save("gd_dqn_FINAL")
    print("✅ Modelo final guardado con éxito como 'gd_dqn_FINAL.zip'")
