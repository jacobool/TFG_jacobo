...
DEATH_FRAMES_NEEDED = 2  # Frames consecutivos sin verde para confirmar muerte (~0.2s a 30fps)
STEP_DURATION = 1 / 8   # Duración fija de cada step (15fps)
MIN_EPISODE_GAP = 1.1

METRICS_PATH = "metrics/gd_metrics_28-3.csv"
SAVE_FREQ = 10000
...
def detect_player_and_band(frame_bgr):
    ...
    if band_bgr.size > 0:
        ...
        if best_cnt is not None and cv2.contourArea(best_cnt) > 50:
            ...
            # Guardamos las coordenadas absolutas
...
def save_plots(history, show=False):
    ...
    plt.tight_layout()
    plt.savefig('plots/metrics_plot_28-3.png', dpi=120)
...
class GDEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        while True:
            ...
            if green_area >= GREEN_AREA_MIN and cooldown_ok:
                ...
                self.attempt += 1
                print(f"Episodio {self.attempt} iniciado. Green area: {green_area}")
                break
            time.sleep(STEP_DURATION)

        obs = self._get_obs(frame_gray, frame_bgr.shape[1], bbox_abs)
        return obs, {}
    ...
    def step(self, action):
        ...
        if action == 1 and (now - self.last_action_time > 0.05):
            ...
            reward -= 0.04  # Penalización mínima por saltar
    ...
    def _capture_frame(self):
        ...
        h, w = img.shape[:2]
        # RECORTE AJUSTADO AL 90% (x2_ai = int(w * 0.90))
    ...
    def _get_obs(self, frame_gray_cropped, original_width, bbox_abs):
        ...
        # Dibujamos el cuadrado blanco sobre el jugador
        ...
        # Redimensionamos a 84x84 (¡El cambio está en esta línea!)
...
if __name__ == "__main__":
    ...
    save_callback = MetricsAndSaveCallback(save_freq=SAVE_FREQ, save_path="modelos_guardados/gd_dqn_5")
