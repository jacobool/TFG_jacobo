...
STEP_DURATION = 1 / 15
MIN_EPISODE_GAP = 1.6
METRICS_PATH = "nave_metrics_FINAL_3.csv"
SAVE_FREQ = 50000
MIN_ACTION_HOLD_STEPS = 4
SWITCH_PENALTY = 0.015
EDGE_PENALTY = 0.05

# Recuperación cuando el jugador no aparece tras una muerte
NO_PLAYER_RECOVERY_SPACE_S = 8.0   # Tras 8s sin verlo: pulsar espacio
NO_PLAYER_RECOVERY_PAUSE_S = 3.0   # Tras 3s más sin verlo: pausar entreno
...
def save_plots(history, show=False):
    ...
    plt.tight_layout()
    plt.savefig('nave_metrics_plot_FINAL_3.png', dpi=120)
...
class GDEnv(gym.Env):
    def __init__(self):
        ...
        # Buffer retroactivo: guarda la obs y reward del primer frame
        # en que el jugador desaparece (colisión real), de forma que
        # cuando se confirma la muerte tras DEATH_FRAMES_NEEDED frames,
        # se devuelve ese estado y no el de la reaparición.
    ...
    def reset(self, seed=None, options=None):
        ...
        no_player_start = time.perf_counter()
        space_recovery_done = False
        recovery_space_time = None
        ...
        while True:
            ...
            player_visible = (green_area >= GREEN_AREA_MIN and green_area <= GREEN_AREA_MAX)

            if player_visible:
                # El jugador está visible: resetear contadores de recuperación
                no_player_start = ahora
                space_recovery_done = False
                recovery_space_time = None

                if (ahora - self.last_episode_time) >= MIN_EPISODE_GAP:
                    self.last_episode_time = ahora
                    self.attempt += 1
                    print(f"🚀 NAVE | Episodio {self.attempt} iniciado. Green area: {green_area}")
                    break
            else:
                elapsed_no_player = ahora - no_player_start

                # Tras NO_PLAYER_RECOVERY_SPACE_S sin jugador: pulsar espacio
                if not space_recovery_done and elapsed_no_player >= NO_PLAYER_RECOVERY_SPACE_S:
                    print(f"⚠️  {NO_PLAYER_RECOVERY_SPACE_S:.0f}s sin detectar al jugador. Pulsando ESPACIO por si hay un popup...")
                    pydirectinput.press("space")
                    space_recovery_done = True
                    recovery_space_time = ahora

                # Tras NO_PLAYER_RECOVERY_PAUSE_S adicionales sin verlo: pausar
                if space_recovery_done and recovery_space_time is not None \
                        and (ahora - recovery_space_time) >= NO_PLAYER_RECOVERY_PAUSE_S:
                    print("\n⏸️  PAUSA: el jugador sigue sin aparecer tras pulsar ESPACIO.")
                    print("   Revisa la pantalla del juego, arregla lo que haga falta,")
                    print("   y pulsa ENTER en esta consola para reanudar el entrenamiento...")
                    try:
                        input()
                    except EOFError:
                        pass
                    print("▶️  Reanudando búsqueda del jugador...")
                    no_player_start = time.perf_counter()
                    space_recovery_done = False
                    recovery_space_time = None
    ...
    def step(self, action):
        ...
        if green_area >= GREEN_AREA_MIN and green_area <= GREEN_AREA_MAX:
            # Jugador visible: resetear buffer
            self._no_green_count = 0
            self._first_no_green_obs = None
            self._first_no_green_reward = None
            ...
            if self._no_green_count == 1:
                # Primer frame sin verde: colisión real.
                # Guardamos la obs y la recompensa de ESTE momento,
                # que es el estado visual que causó la muerte.
                self._first_no_green_obs = self._get_obs(
                    frame_color_cropped, frame_bgr.shape[1], bbox_abs
                )
                # La recompensa en el frame de colisión es 0 (ni supervivencia
                # ni penalización: la penalización se aplica al confirmar)
                self._first_no_green_reward = 0.0
                reward = 0.0
                obs = self._first_no_green_obs

            elif self._no_green_count < DEATH_FRAMES_NEEDED:
                # Frames de tolerancia intermedios
                reward = 0.0
                obs = self._first_no_green_obs  # Seguimos mostrando el frame de colisión

            else:
                # Muerte confirmada: devolvemos el estado del frame de colisión real,
                # no el estado actual (que podría ser la reaparición del jugador)
                reward = -5.0
                done = True
                self._no_green_count = 0
                obs = self._first_no_green_obs  # ← Frame retroactivo de la colisión
                self._first_no_green_obs = None
                self._first_no_green_reward = None
    ...
    def _get_obs(self, frame_color_cropped, original_width, bbox_abs):
        # 1. Filtro de blancos (obstáculos)
...
if __name__ == "__main__":
    ...
    save_callback = MetricsAndSaveCallback(
        save_freq=SAVE_FREQ,
        save_path="modelos_guardados/nave_dqn_3"
    )
    ...
    print("🚀 Iniciando entrenamiento del PILOTO (Modo Nave) — v2 con buffer retroactivo.")
    print(f"   Guardado automático cada {SAVE_FREQ} pasos.")
    print("   Deja el juego en primer plano y no muevas el ratón...")

    model.learn(total_timesteps=900000, callback=save_callback)

    print("\n📊 Entrenamiento finalizado.")
    model.save("modelos_guardados/nave_dqn_FINAL_3")
    print("Modelo 'nave_dqn_FINAL_3' guardado.")
