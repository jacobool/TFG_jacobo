...
STEP_DURATION = 1 / 15
MIN_EPISODE_GAP = 1.6
METRICS_PATH = "nave_metrics_QRDQN.csv"
PLOT_PATH = "nave_metrics_plot_QRDQN.png"
SAVE_FREQ = 50000
...
def save_plots(history, show=False):
    ...
    axs[0].set_title('Tiempo de Supervivencia (NAVE — QR-DQN)')
    ...
    axs[2].set_title('Pérdida (Quantile Huber Loss) Promedio')
        ...
        while True:
            ...
            if player_visible:
                ...
                if (ahora - self.last_episode_time) >= MIN_EPISODE_GAP:
                    ...
                    self.attempt += 1
                    print(f"🚀 NAVE QR-DQN | Episodio {self.attempt} iniciado. Green area: {green_area}")
                    break
            else:
    ...
    def _on_step(self) -> bool:
        ...
        if self.locals["dones"][0]:
            ...
            if hasattr(self.model, "logger"):
                # QR-DQN registra la pérdida bajo "train/loss" (quantile huber loss)
...
if __name__ == "__main__":
    ...
    save_callback = MetricsAndSaveCallback(
        save_freq=SAVE_FREQ,
        save_path="modelos_guardados/nave_qrdqn"
    )

    # QR-DQN: DQN distribucional. Aprende la distribución de Q-values
    # mediante 170 cuantiles, lo que ayuda con recompensas con mucha
    # varianza (en este entorno: +0.08 vivo / -5.0 muerte súbita).
    model = QRDQN(
        "CnnPolicy", env,
        verbose=1,
        buffer_size=50000,
        ...
        policy_kwargs=dict(n_quantiles=170),
    )

    print("🚀 Iniciando entrenamiento del PILOTO (Modo Nave) — QR-DQN.")
    print(f"   Guardado automático cada {SAVE_FREQ} pasos.")
    print("   Deja el juego en primer plano y no muevas el ratón...")

    model.learn(total_timesteps=450000, callback=save_callback)

    print("\n📊 Entrenamiento finalizado.")
    model.save("modelos_guardados/nave_qrdqn_FINAL")
    print("Modelo 'nave_qrdqn_FINAL' guardado.")
