...
"""Baseline scratch para la PARTE 2 del cubo.

QR-DQN entrenado desde cero, sin profesor ni transferencia. Sirve como
referencia para comparar contra:
  - fine-tuning desde la parte 1 (gd_rl_env_4_qrdqn_finetune_cubo2.py)
  - indirect transfer via distillation (gd_rl_env_4_qrdqn_distill_cubo2.py)

Mismos hiperparametros y total de timesteps que el estudiante del script
de distillation, para que la comparacion sea justa.

Antes de ejecutar:
  - Posiciona al jugador al inicio de la PARTE 2 del cubo.
"""
...
METRICS_DIR = "metrics"
RUN_NAME = "qrdqn_scratch_nivel2"

TOTAL_TIMESTEPS = 80000
SAVE_FREQ = 5000

# Mismos hiperparametros que el estudiante de distillation, sin profesor.
LR = 5e-5
BUFFER = 30000
...
if __name__ == "__main__":
    ...
    print(f"[SCRATCH] LR={LR}  buffer={BUFFER}  total={TOTAL_TIMESTEPS}")
    print(f"[SCRATCH] Metricas -> {METRICS_DIR}/{RUN_NAME}_metrics.csv, "
    ...
    env.close()
    print("[SCRATCH] Entreno scratch terminado.")
