...
# --- 1. CONFIGURACIÓN EXACTA V2 (Debe coincidir con el entrenamiento 2) ---
...
GREEN_AREA_MIN = 130  
GREEN_AREA_MAX = 300 # ¡Nuevo de la v2!
...
# Físicas y Ritmo de la v2
STEP_DURATION = 1 / 15  # ¡Ahora a 15 FPS!
MIN_ACTION_HOLD_STEPS = 5 # Bloqueo anti-parkinson
...
class GDEnvPlay(gym.Env):
    ...
    def step(self, action):
        ...
        # --- Lógica de Control Anti-Oscilación (Igual que en el entreno v2) ---
        ...
        # Muerte
        done = green_area < GREEN_AREA_MIN or green_area > GREEN_AREA_MAX

        obs = self._get_obs(frame_color_cropped, frame_bgr.shape[1], bbox_abs if not done else None)
        # Respetamos el lag de la IA (15 FPS)
...
if __name__ == "__main__":
    print("⏳ Cargando el cerebro de la Nave v2...")
    
    env = DummyVecEnv([lambda: GDEnvPlay()])
    env = VecFrameStack(env, n_stack=4)
    
    MODELO_A_PROBAR = "modelos_guardados/nave_dqn_3_450000_steps"
    
    try:
        model = DQN.load(MODELO_A_PROBAR)
        print(f"✅ Cerebro '{MODELO_A_PROBAR}' cargado correctamente.")
    except FileNotFoundError:
        print(f"❌ ERROR: No se encuentra '{MODELO_A_PROBAR}.zip'.")
        exit()
